...
export const GymPreviewCard: React.FC<GymPreviewCardProps> = ({
  ...
  return (
    <div className="mt-4" onClick={onClick}>
      ...
      <div className="border border-green/20 bg-shadow rounded-md px-3 py-1 text-green shadow-md w-full overflow-hidden">
        ...
        <div className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,auto)_minmax(0,107px)_minmax(0,100px)] gap-2 items-stretch min-w-0">
          {/* Gym name — wraps */}
          <div className="flex items-center border-r border-green/20 font-semibold pr-2 break-words whitespace-normal min-w-0">
            {gym.name}
          </div>

          {/* Email — truncate and respect padding */}
          <div className="flex items-center border-r border-green/20 text-sm font-semibold pr-2 overflow-hidden min-w-0">
